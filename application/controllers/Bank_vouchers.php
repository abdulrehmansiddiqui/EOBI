...
class Bank_vouchers extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['account'] = $this->invoice_model->get_chart_account();
        $data['get_fixed'] = $this->items_model->get_fixed();
        $this->load->view('bank_vouchers', $data);
...
//        var_dump($sales_data);die;
        $sales_id = $this->invoice_model->bpv_insert($sales_data);
        if($insert_type == "Add & Print"){
            $received_from = $this->accounts_model->get_account($sales_data['received_from']);
            $sent_to = $this->accounts_model->get_account($sales_data['sent_to']);
            $data['details'] = $sales_data;
            $data['received_from'] = $received_from;
            $data['sent_to'] = $sent_to;
            $this->load->view('Reports/generated_reports',$data);
//            $html = $this->load->view('Reports/generated_reports',$data,true);
//            $dompdf = new Dompdf();
